...
def connect_with_fallback(ips: list[str], port: int, timeout: float, prefer: str = "any") -> TCPConnectResult:
    ...
    ordered = [ip for ip in ips if ":" not in ip] + \
                [ip for ip in ips if ":" in ip] if prefer == "ipv4" else \
              [ip for ip in ips if ":" in ip] + \
                [ip for ip in ips if ":" not in ip] if prefer == "ipv6" else \
              ips
    ...
    for ip in ordered:
        ...
        if s is not None: # connection success!
    ...
    # 여기까지 왔으면 그냥 실패
    return TCPConnectResult( 
        ip=ordered[-1] if ordered else None, port=port, connect_ms=None,
        local_addr=None, peer_addr=None,
        error=last_err or "All connections failed", sock=None
    )